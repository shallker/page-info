...
module.exports = function pageInfo(url, callback, onError) {
  ...
  function requestPage(requestUrl, callback, onError) {
    var http = new Http;
    ...
    http.on(301, function (response) {
      ...
      if (redirection.match(/^\//)) {
        redirection = url.protocol + '//' + url.host + redirection;
      }

      requestPage(redirection, callback, onError);
    });
